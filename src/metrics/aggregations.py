...
def aggregate_xg_by_team(events, conn=None, match_id=None) -> pd.DataFrame:
    ...
    if isinstance(events, str):
        ...
        SELECT 
            ...
            ROUND(SUM(shot_statsbomb_xg), 3) as total_xg,
            ROUND(SUM(CASE WHEN shot_type != 'Penalty' THEN shot_statsbomb_xg ELSE 0 END), 3) as npxg,
            SUM(CASE WHEN shot_outcome = 'Goal' THEN 1 ELSE 0 END) as total_goals,
            SUM(CASE WHEN shot_outcome = 'Goal' AND shot_type != 'Penalty' THEN 1 ELSE 0 END) as np_goals,
            ROUND(AVG(CASE WHEN shot_type != 'Penalty' THEN shot_statsbomb_xg END), 3) as avg_npxg_per_shot
        FROM '{events}'
        WHERE type = 'Shot' AND shot_statsbomb_xg IS NOT NULL {match_filter}
        GROUP BY match_id, team
        ORDER BY match_id, npxg DESC
        """
...
def aggregate_xg_by_player(events, conn=None, match_id=None, min_shots=3) -> pd.DataFrame:
    ...
    if isinstance(events, str):
        ...
        SELECT 
            ...
            ROUND(SUM(CASE WHEN shot_outcome = 'Goal' THEN 1 ELSE 0 END) - SUM(shot_statsbomb_xg), 2) as goals_minus_xg
        FROM '{events}'
        WHERE type = 'Shot' AND player IS NOT NULL {match_filter}
...
def get_match_summary(events, matches, conn=None, match_id=None) -> pd.DataFrame:
    ...
    if isinstance(events, str):
        ...
        # Pandas logic for match summary
