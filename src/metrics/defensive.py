...
"""Defensive contribution metrics - tackles, interceptions, pressures, blocks."""
...
def calculate_defensive_actions(
    ...
    Counts tackles, interceptions, pressures, blocks, clearances, and 
    calculates defensive actions by pitch zone.
    ...
    Returns:
        DataFrame with defensive action counts per player
    """
    if isinstance(events, str):
        if conn is None:
            conn = duckdb.connect()
        ...
        # Filter to players with at least 1 defensive action
...
def calculate_defensive_profile(
    ...
    Defensive profiles:
    - Aggressive Presser: High pressures, high turnovers
    - Ball Winner: High tackles + interceptions
    - Protector: High blocks + clearances
    - Dominant Duelist: High duel win %
    - Positional Defender: Low actions but strategic positioning
    
    Args:
        events: Events data
        conn: DuckDB connection
        match_id: Optional match filter
        min_actions: Minimum defensive actions to include
        
    Returns:
        DataFrame with defensive profile classification
    """
    # Get defensive actions
    defensive = calculate_defensive_actions(events, conn, match_id)
